...
/*
 * This class manages the initialization of Temperature device
 * 
	...
	public static void setDeviceInitialData(String contextPath, int deviceId) {
		try {
			BufferedWriter out = new BufferedWriter(new FileWriter(contextPath
					+ "/config/temperatureDevice" + deviceId + ".xml"));
			// Get the last 60 values for temperature device. (last hour)
			ArrayList<DeviceReading> readings = DbSupport
					.getLastInputsByDevice(deviceId, 60);
